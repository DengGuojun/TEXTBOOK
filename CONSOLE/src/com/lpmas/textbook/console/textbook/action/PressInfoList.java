...
@WebServlet("/textbook/PressInfoList.do")
public class PressInfoList extends HttpServlet {
	public static final long serialVersionUID = 1L;

	public PressInfoList() {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		HashMap<String, String> condMap = ParamKit.getParameterMap(request, "pressName", "");
		condMap.put("status", String.valueOf(Constants.STATUS_VALID));

		PressInfoBusiness business = new PressInfoBusiness();
		PageResultBean<PressInfoBean> result = business.getPressInfoPageListByMap(condMap, pageBean);

		request.setAttribute("pressInfoList", result.getRecordList());
		pageBean.init(pageNum, pageSize, result.getTotalRecordNumber());
		request.setAttribute("PageResult", pageBean);
		request.setAttribute("CondList", MapKit.map2List(condMap));
		request.setAttribute("AdminUserHelper", adminUserHelper);
		RequestDispatcher rd = request
				.getRequestDispatcher(TextbookConsoleTextbookConfig.PAGE_PATH + "PressInfoList.jsp");
